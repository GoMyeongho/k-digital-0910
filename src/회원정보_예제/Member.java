...
public class Member {
    ...
    private String name;        // private : 접근 제한자, 같은 클래스 내에서만 접근 가능
    private int age;
    private char gender;
    private int job;
    ...
    public void setGender(){
        while (true){
            ...
            gender = sc.next().charAt(0);       // 해당 인덱스의 문자 추출
            switch (gender){
                case 'M': case 'm':
                case 'F': case 'f':
                    return;
    ...
    public int getGenderType() {    // 반환타입이 존재함(int)
        if (gender == 'M' || gender =='m') return 1;
        return 2;
    }

    public void getInfo() {
        String[] genderStr = {"", "남성", "여성"};  // 입력된 값과 배열의 인덱스를 일치시키기 위해서 첫번째를 비움
